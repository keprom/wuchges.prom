...
<table width=100% border=1px cellspacing=0px style="border: black;font-family: Verdana; font-size: xx-small;">
    ...
    <?php foreach ($sql_result->result() as $data): ?>
        <tr>
            ...
            <td align=right>
                <?php if (!$data->nachisleno) {
                    echo "0.00";
                } else {
                    echo f_d($data->nachisleno);
                } ?>
            </td>
            <td align=right>
                <?php if ($nazv == " дебиторов ") {
                    if (!$data->debet_value) {
                        ...
                        echo f_d($data->debet_value);
                    }
                } else {
                    if (!$data->kredit_value) {
                        echo "0.00";
                    } else {
                        echo f_d($data->kredit_value);
                    }
                } ?>
            </td>
        </tr>
    ...
    <?php endforeach; ?>
    <tr align=right>
        <td colspan=5 align=left><b>Итого:</b></td>
        <td><b><?php echo f_d($sum_nachisleno); ?></b></td>
        <td><b><?php if ($nazv == " дебиторов ") {
                    echo f_d($sum_debet);
                } else {
                    echo f_d($sum_kredit);
                } ?></b></td>
